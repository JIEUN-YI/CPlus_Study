    ...
    _In_ int       nCmdShow)
{
    ...
    // 기본 메시지 루프입니다:
    // 기본 메시지 루프를 가지고 게임을 만들기 부적합한 이유
    // 1. 그리기 작업이 실시간으로 진행되지 않는다 -> 윈도우가 바로 그리는 것이 아닌 그려야 할때만 그린다
    //  => 게임은 아무런 입력이 없어도 실시간으로 그려져야 하는데, 지금은 입력이 있어야지만 그려진다는 것
    // 2. 누르고 있는 경우 바로 반응해서 그려야하는데 -> 윈도우 메시지 방식은 똑... 또로로로로 방식
    //  => 동시 입력이 X + 누르자 마자 반응이 아닌, 키보드의 입력을 기다리는 방식

    // GetMessage : 메시지가 없으면 대기
    // MSG msg;
    ...
    // 게임 메시지 루프입니다:
    // 게임 메시지 루프는 게임에 적합하게 메시지 루프 방식을 변경
    // 1. 안누르고 있는 경우도 동작
    // 2. 동시 입력을 확인할 수 있고, 바로 반응해야 한다.

    Game game;
    game.Start(hInst, hWnd);
    MSG msg;
    // PeekMessage : 메시지가 없으면 넘어가고 있으면 꺼내주기
    while (true)
    {
        // 메세지가 없으면 게임처리, 메시지가 있으면 메세지처리
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))            // 메시지가 있으면 메세지처리
        {
            // 메세지가 프로그램을 종료하라는 메시지 일때
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpfnWndProc = WndProc;                      // 메시지를 받는 함수를 지정
    wcex.cbClsExtra = 0;
    wcex.cbWndExtra = 0;
    ...
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);   // 마우스 커서 아이콘 설정
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);         // 배경 색상
    wcex.lpszMenuName = nullptr; // MAKEINTRESOURCEW(IDC_WINAPI);     // 윈도우 메뉴 설정
...
/// </summary>
/// <param name="hWnd"> 메시지가 어느 윈도우 창이 선택됐는지 구분 </param>
/// <param name="message"> 메세지의 종류 </param>
/// <param name="wParam"> 메시지의 추가 정보1 </param>
/// <param name="lParam"> 메시지의 추가 정보2 </param>
/// <returns></returns>
...
/* rodls tndjq wlsgod djeltj xmffuTsmswl ahfmrpTsms zhem */
//   // WinAPI_Study.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//   //
//   
//   #include "framework.h" // WinAIP에서 필요로 하는 추가 기능들
